...
if __name__ == '__main__':
    ...
    while True:
        ...
        Q = Quaternion(IMU.rotationVector.real, IMU.rotationVector.i, IMU.rotationVector.j,
                       IMU.rotationVector.k)  # 获取OAK相机旋转向量
        ...
        rd = Q.rotate(
            np.array([-1000 * math.tan(math.radians(vfov / 2)), -1000 * math.tan(math.radians(hfov / 2)), 1000]))
        # cv2.line(background, (R, R), (int(lu[0] / 10 + 0.5) + R, - int(lu[1] / 10 + 0.5) + R), (0, 150, 0), 1)
        # cv2.line(background, (R, R), (int(ld[0] / 10 + 0.5) + R, -int(ld[1] / 10 + 0.5) + R), (0, 0, 150), 1)
        # cv2.line(background, (R, R), (int(ru[0] / 10 + 0.5) + R, -int(ru[1] / 10 + 0.5) + R), (0, 150, 0), 1)
        # cv2.line(background, (R, R), (int(rd[0] / 10 + 0.5) + R, -int(rd[1] / 10 + 0.5) + R), (0, 0, 150), 1)

        depthDemo = cv2.applyColorMap(cv2.equalizeHist(cv2.normalize(D, None, 255, 0, cv2.NORM_INF, cv2.CV_8UC1)),
